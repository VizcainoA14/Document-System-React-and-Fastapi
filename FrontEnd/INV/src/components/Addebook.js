...
export default function Addeebook() {
    ...
    const [editor, setParam9] = useState('');
    const [pages, setParam10] = useState(1);
    const [synopsis, setParam11] = useState('');
    ...
    const handleSubmit = (event) => {
        ...
        fetch(`http://127.0.0.1:8000/api/document/ElectronicDoc/Add/Ebook/${author}/${title}/${price}/${topic}/${language}/${pub_date}/${size}/${doi}/${editor}/${pages}/${synopsis}`, {method: 'POST'})
    ...
    return (
        <div className='Form'>
            ...
            <aside><Delete data="Ebooks"/></aside>
        </div>
    )
}
